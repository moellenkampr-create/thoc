# Import: 2077-07-05-ely-oggy-aldecaldos

## Meta
- Import-ID: 2077-07-05-ely-oggy-aldecaldos
- Datum: 2077-07-05
- Quelle: lore/raw/lore_import_2077-07-05_ely_report.md
- Vorlaeufiger Statusmix: ARBEITSSTAND (Ely-Sicht), KANON-Kandidaten enthalten

## Ely-Sicht
- Kernwahrnehmung:
  - Morgen nach Sunset-Motel-Nacht mit Erschoepfung, dann Rueckkehr in Missionsmodus.
  - Uebergabe von Datenstick bei Dakota, Kontaktanbahnung zu Mitch/Aldecaldos.
  - Oggy-Schrottplatz mit Kurierbike, Scavenger-Beobachtung und weiterem Stickfund.
  - Strategischer Weitertransport des Bikes Richtung Aldecaldos, inklusive Tankstellenstopp und Beobachtung eines Anzugtyps.
  - Zukunftsgespraech im Trio ueber Uni, Football und Lebensausrichtung.
- Emotionale Marker:
  - Fuersorge fuer Lauri wegen verdeckter Schmerzen.
  - starke Anziehung und Verbundenheit im Trio.
  - Unruhe und Misstrauen gegenueber Scavengers und Anzugtyp.
  - wachsender Entscheidungsdruck zur Zukunft (Uni/Football/Lebensweg).
- Schluesselformulierungen / Zitate:
  - "Endlich keine Schmerzen mehr"
  - "Oh Snaki, warum hast du nichts gesagt?"
  - "Das haettet ihr auch einfacher haben koennen. Ich bin Mitch von den Aldecaldos"
  - "Donny?" / "Spaeter!"
  - "Einmal ist keinmal, zweimal ein Zufall, aber dreimal ein Muster."
- Fakten:
  - Storyzeitpunkt: 2077-07-05 (Ely-Sicht).
  - George bringt morgens Kaffee, Snacks und "Shower in the Can".
  - Das Trio liefert einen Stick bei Dakota ab, erhaelt Koordinaten und trifft Mitch.
  - Beim Kurierbike bei Oggy wird ein weiterer Datenstick am Reservetank gefunden und vorerst fuer Donny zurueckgehalten.
  - Scavengers erscheinen am Schrottplatz; sie ziehen wieder ab.
  - Das Trio transportiert das Bike Richtung Aldecaldos.
  - Tankstellenstopp und verdeckte Beobachtung eines Anzugtyps mit zwei Begleitern bei Oggy.
  - Spaetere Recherche benennt den Mann als **Harry Bohn**, Privatsekretaer von "Primus Belsarius Prime" (Schreibweise ungesichert).
  - Auf der Route wird das Trio von Aldecaldos gestellt; Mitch klaert die Lage, Bike wird verladen.
  - Im Trio gibt es offene Zukunftsdebatte:
    - Lauri erwaegt Uniabbruch und Werkstatt-/Schrauberweg.
    - George erwaegt Ausstieg aus Football trotz emotionaler Bindung.
    - Ely will an der Uni bleiben (Chemie/Experimente).
- Unsicherheiten:
  - Schreibweise und genaue Identitaet: "Primus Belsarius Prime" vs. etablierter Name im Bestand.
  - Exakte Reihenfolge einzelner Oggy-/Tankstellen-/Aldecaldo-Szenen.
  - Dateninhalt des neu gefundenen Sticks.
  - Aussagen sind bislang nur in Ely-Sicht belegt.

## Lauri-Sicht
- Kernwahrnehmung: offen
- Emotionale Marker: offen
- Schluesselformulierungen / Zitate: offen
- Fakten: offen
- Unsicherheiten: komplette Sicht fehlt

## George-Sicht
- Kernwahrnehmung: offen
- Emotionale Marker: offen
- Schluesselformulierungen / Zitate: offen
- Fakten: offen
- Unsicherheiten: komplette Sicht fehlt

## Spielleiter-/Erzaehler-Sicht
- Metaeinordnung:
  - Ue18-Rahmen gilt; fuer Chroniken bleiben Erotikmarker erhalten, pornografische Details nicht.
- Harte Weltfakten: vorlaeufig offen bis Gegenabgleich
- Dramaturgische Absicht: vorlaeufig offen
- Steuernde Hinweise: vorlaeufig offen
- Unsicherheiten / offene Hooks:
  - Mehrfach-Interesse am Bike/Stick deutet auf groesseren Konfliktfaden.

## Konsolidierung
- Gemeinsame Schnittmenge:
  - Nur Ely-Sicht vorhanden, daher vorlaeufig kein Mehrperspektiven-Kern.
- Perspektivkonflikte:
  - Noch nicht pruefbar.
- Statusentscheidungen:
  - Neue 07-05-Details bleiben `ARBEITSSTAND` bis Gegenabgleich.
  - Keine stille Kanonisierung.

## Zielmapping
- lore_world: keine unmittelbare Aenderung
- lore_characters: Ely/Lauri/George Zukunftsdruck und Rollenwandel (ARBEITSSTAND)
- lore_factions: Aldecaldos-/Scavenger-/Milieu-Interaktion am 07-05 (ARBEITSSTAND)
- lore_relationships: Trio-Fuersorge und Spannungsregulation (ARBEITSSTAND)
- lore_timeline: 2077-07-05 Morgen bis Route zu Aldecaldos aus Ely-Sicht (ARBEITSSTAND)
- lore_locations: Oggys Schrottplatz, Tankstelle, Route zu Aldecaldos (ARBEITSSTAND)
- lore_plots: Stick-/Bike-/Anzugtyp-/Aldecaldo-Verknuepfung (ARBEITSSTAND)
- lore_open_threads: Identitaet/Schreibweise um Primus/Harry Bohn und Stickinhalt offen
- lore_flair_quotes: markante 07-05-Formulierungen und Tonanker

## Offene Fragen an Spielleitung
- Ist "Primus Belsarius Prime" ein bewusster Name oder ein Ely-/Datenfehler gegenueber "Primus Belsarius Trump"?
- Was genau steckt auf dem zweiten Stick aus dem Reservetank?
- Soll der Zukunftskonflikt Uni/Football/Schrauberweg als eigener Plotfaden hochgezogen werden?